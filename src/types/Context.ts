...
type Context<Columns extends Table['columns']> = {
    col: <columnKey extends keyof Columns & string>(
        column: columnKey,
        alias?: string
    ) => ValueExpression<ColumnTypeByColumns<Columns, columnKey>>;
    ...
    colLike: <
        ...
        Op extends string extends ColumnTypeByColumns<Columns, columnKey> ? LikeOperator : never
    >(
        column: columnKey,
        op: Op,
        v: Op extends 'like'
    ...
    colJson: <
        columnKey extends keyof Columns & string,
        Op extends JSON extends ColumnTypeByColumns<Columns, columnKey> ? JsonOperator : never
    >(
        column: columnKey,
        op: Op,
        v: Op extends '@>' | '<@'
...
type ContextScope<Columns extends Table['columns']> = (
    rules: {
...
type CompareOperatorCompatible<T, True, False> = number extends T
    ? True
    : bigint extends T
    ? True
    : Decimal extends T
    ...
    ? True
    : False;
type ListOperatorCompatible<T, True, False> = number extends T
    ? True
    : bigint extends T
    ? True
    : Decimal extends T
