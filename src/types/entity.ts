...
type CustomColumn<Exp extends Expression<ExpressionTypes>, As extends string> = { exp: Exp, as: As };
...
type InsertValue<Columns extends Table['columns'], NullableAndDefaultCols extends readonly (keyof NullableAndDefaultColumns<Columns>)[]> =
    ...
    &
    {
        [columnKey in Exclude<keyof NullableAndDefaultCols, keyof unknown[]> as NullableAndDefaultCols[columnKey] & string]?:
        Expression<ColumnTypeByColumns<Columns, NullableAndDefaultCols[columnKey] & string>>;
    };
type UpdateSets<Columns extends Table['columns']> = { [columnKey in keyof Columns & string]?: Expression<ColumnTypeByColumns<Columns, columnKey>>; };
...
type QueryResultRow<Columns extends Table['columns'], Returning extends readonly (keyof Columns | CustomColumn<Expression<ExpressionTypes>, string>)[]> = {
    ...
    Returning[key] extends CustomColumn<infer Exp, string> ? Exp extends Expression<infer ExpType> ? ExpType : Exp : ColumnTypeByColumns<Columns, Returning[key] & keyof Columns>;
};
type PartialQuery = { text: string, params: Param[] };
