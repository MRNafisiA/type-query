...
const createContext = <T extends Table>(table: T, alias: string = ''): Context<T['columns']> => {
    ...
    return {
        ...
        colList: (column, op, v, _alias = alias) => U.listOp(U.col(table, column, !!_alias, _alias) as any, op, v as any),
        colLike: (column, op, v, _alias = alias) => U.likeOp(U.col(table, column, !!_alias, _alias) as any, op as any, v as any),
        colJson: (column, op, v, _alias = alias) => U.jsonOp(U.col(table, column, !!_alias, _alias) as any, op as any, v as any),
...
const createContextScopeHelper = <T extends Table>(table: T) =>
    (rules: Parameters<ContextScope<T['columns']>>[0], alias: string) => Object.entries(rules).map(([key, value]) => {
        switch (value[0]) {
            case '= null':
            case '!= null':
